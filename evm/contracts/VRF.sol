...
contract VRF {
  ...
  */
  function projectiveECAdd(uint256 x1, uint256 y1, uint256 x2, uint256 y2)
    ...
    // TODO(alx): These calculations could be optimized slightly to take
    // advantage of the fact that z1=z2=1, at the expense of some readability.

    // Add extra "projective coordinate" to the two points
    (uint256 z1, uint256 z2) = (1, 1);

    // (lx, lz) = (y2-y1)/(x2-x1), i.e., gradient of secant line.
    (uint256 lx, uint256 lz) = (addmod(y2, FIELD_SIZE - y1, FIELD_SIZE),
                                addmod(x2, FIELD_SIZE - x1, FIELD_SIZE));
  ...
  function scalarFromCurve(uint256[2] memory _hash, uint256[2] memory _pk,
                           uint256[2] memory _gamma, address _uWitness,
                           uint256[2] memory _v)
    public pure returns (uint256 s) {
    bytes32 iHash = keccak256(abi.encodePacked(_hash, _pk, _gamma, _uWitness,
                                               _v));
    return zqHash(GROUP_ORDER, uint256(iHash));
  }

  // True if (gamma, c, s) is a correctly constructed randomness proof from _pk
  // and _seed. _zInv must be the inverse of 
  function verifyVRFProof(uint256[2] memory _pk,
    ...
    uint256[2] memory v = linearCombination(_c, _gamma, _cGammaWitness, _s,
                                            hash, _sHashWitness, _zInv);
  ...
  */
  function isValidVRFOutput(uint256[2] memory _pk, uint256[2] memory _gamma,
                            uint256 _c, uint256 _s, uint256 _seed,
                            address _uWitness, uint256[2] memory _cGammaWitness,
                            uint256[2] memory _sHashWitness,
                            uint256 _zInv, uint256 _output)
    public view returns (bool) {
